...
export default function ModeToggle() {
  const { theme, toggleTheme } = useAppContext(); // Access theme and toggleTheme from context

  return (
    <button
      onClick={toggleTheme}
      className="text-white border-gray-800 border-2 dark:bg-gray-800 dark:border-none p-2 m-1 rounded-full"
    >
      {theme === 'dark' ? (
